...
it is enough to prove it for the empty set (`H0`) and to prove
(`H1`) that if $s$ is a finite set and $a ∉ s$ then $P(s) \rightarrow P(\{ a \} \cup s)$.

finite.induction_on {P : set α → Prop} {s : set α} (h : finite s)
  (H0 : P ∅) (H1 : ∀ {a s}, a ∉ s → finite s → P s → P (insert a s)) : P s
-/

/- Axiom : mem_insert_iff

We have $x ∈ \{a\} \cup s$ if and only if $x =a$ or $x \in s$.
